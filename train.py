...
def unnormalize(img):
    """
    Unnormalize a batch of images from CIFAR-10 normalization.

    Args:
    img (torch.Tensor): Batch of normalized images with shape [B, C, H, W]

    Returns:
    torch.Tensor: Unnormalized images in the range [0, 1]
    """
...
def train_epoch(model, discriminator, train_loader, criterion_adv, lambda_rec, lambda_adv,
    ...
    for i, (images, _) in pbar:
        ...
        outputs = nn.functional.interpolate(outputs, size=(227, 227), mode='bilinear', align_corners=False)
        fake_logits = discriminator(outputs)
        ...
        loss_rec_masked = (diff * inverse_mask).sum() / (inverse_mask.sum() + 1e-8)
        # Reconstruction loss on unmasked region
        loss_rec_unmasked = (diff * mask).sum() / (mask.sum() + 1e-8)
        # Total generator loss with unmasked region termi
        loss_g = lambda_rec * loss_rec_masked + lambda_adv * loss_adv + args.lambda_unmasked * loss_rec_unmasked
        ...
        if i % 200 == 0:
            img_grid_original = unnormalize(images[:4])
            img_grid_masked = unnormalize(masked_images[:4])
            img_grid_recon = unnormalize(outputs[:4])
...
def validate(model, val_loader, criterion, epoch, args, writer):
    ...
    if len(val_loader) > 0:
        with torch.no_grad():
            ...
            images_og = unnormalize(images)
            masked_images_og = unnormalize(masked_images)
            outputs_og = unnormalize(outputs)

            writer.add_images('val_original', images_og, epoch)
            writer.add_images('val_masked', masked_images_og, epoch)
            writer.add_images('val_reconstructed', outputs_og, epoch)
...
def main():
    ...
    optimizer_g = optim.AdamW(model.parameters(), lr=args.lr, betas=(0.5, 0.999))
...
if __name__ == "__main__":
    main()
e2gu@dsmlp-login:~/ece176final$ vim train.py
e2gu@dsmlp-login:~/ece176final$ cat train.py 
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
